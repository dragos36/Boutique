...
$result = $db->query($sql);//we need this for the while loop which is down
$errors = array();
$category = '';
$post_parent = '';
...
if(isset($_GET['delete']) && !empty($_GET['delete'])){
  $delete_id = (int)$_GET['delete'];//bcs is a get is taking the url which contains the id as well and is putting this into $delete_id
...
if(isset($_POST) && !empty($_POST)){
  ...
  if(!empty($errors)){
    //display errors
    $display = display_errors($errors); ?><!--why is like that ?>-->
    <script>
    ...
    <form class="form" action="categories.php <?=((isset($_GET['edit']))?'?edit='.$edit_id:''); ?>" method="post">
      ...
      <div class="form-group">
        ...
        <select class="form-control" name="parent" id="parent">
          <option value="0"<?=(($parent_value == 0)?' selected = "selected"':''); ?>>Parent</option><!--is 0 bcz in the database the parent has id 0-->
